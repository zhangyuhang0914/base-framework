...
export const validateTelephone = (rule, value, callback) => {
  if (value === '' || value === null || typeof value === 'undefined') {
    ...
    if (/^([+]?[\d]{1,3}-|[ ])?((13[0-9]{9})|(18[0-9]{9})|(19[0-9]{9})|(15[0-9]{9})|(16[0-9]{9})|(14[0-9]{9})|(17[0-9]{9}))$/.test(value)) { // 验证手机
      callback()
...
// 只能输入英文字母、数字、下划线
export const validateEnNumUdl = (rule, value, callback) => {
  if (value === '') {
    callback()
  } else {
...
// 不能输入空格
export const validateBlack = (rule, value, callback) => {
  if (value === '' || value.trim() === '') {
...
export const validatePositiveDoubleNumber = (rule, value, callback) => {
  if (!value) {
    callback()
  } else {
    // if (/^[-\+]?\d+(\.\d+)?$/.test(value)) {
...
export const checkPostal = (rule, value, callback) => {
  var reg = /^[0-9]{6}$/
  if (reg.test(value)) {
    callback()
  } else if (value === '' || value.length === 0) {
    if (rule && rule.require) { callback(new Error('请输入您的邮政编码')) }
    callback()
  } else {
...
export const validatePassword = (rule, value, callback) => {
  ...
  if (value === '') {
    ...
    if (reg.test(value)) {
      ...
      callback(new Error('密码长度为8~20个字符，必须同时包含数字、大写字母和小写字母'))
    }
  }
}
