...
export const validateEmail = (
  ...
  if (value === '') {
    ...
    if (
      /^[A-Za-z\d]+([-_.][A-Za-z\d]+)*@([A-Za-z\d]+[-.])+[A-Za-z\d]{2,6}$/.test(
        value
      )
    ) {
      callback()
...
export const validateTelephone = (
  ...
  if (value === '' || value === null || typeof value === 'undefined') {
    ...
    if (
      /^([+]?[\d]{1,3}-|[ ])?((13[0-9]{9})|(18[0-9]{9})|(19[0-9]{9})|(15[0-9]{9})|(16[0-9]{9})|(14[0-9]{9})|(17[0-9]{9}))$/.test(
        value
      )
    ) {
      // 验证手机
...
export const validateMobile = (
  ...
  if (value === '') {
    ...
    if (
      /^([+]?[\d]{1,3}-|[ ])?((13[0-9]{9})|(18[0-9]{9})|(15[0-9]{9})|(16[0-9]{9})|(14[0-9]{9})|(17[0-9]{9})|(19[0-9]{9}))$/.test(
        value
      )
    ) {
      callback()
  ...
  if (!value) {
    ...
    if (/^-?\d+(\.\d+)?$/.test(value)) {
      ...
      if (rule.min !== undefined) {
        val >= rule.min
          ? (result.min.result = true)
          : callback(new Error(result.min.msg))
      }
      if (rule.max !== undefined) {
        val <= rule.max
          ? (result.max.result = true)
          : callback(new Error(result.max.msg))
      }
      if (rule.int) {
        ;/^(0|[1-9][0-9]*|-[1-9][0-9]*)$/.test(value)
          ? (result.int.result = true)
          : callback(new Error(result.int.msg))
      }
...
export const validateDoubleNumber = (
  rule: AnyObject,
  value: string,
  callback: (error?: Error) => void
) => {
...
export const validatePositiveDoubleNumber = (
  rule: AnyObject,
  value: string,
  callback: (error?: Error) => void
) => {
