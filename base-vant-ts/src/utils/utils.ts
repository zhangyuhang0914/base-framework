...
/**
 * 时间格式化 YYYY-MM-DD HH:mm:ss 格式
 * @param str
 * @returns YYYY-MM-DD HH:mm:ss
 */
export const formatToYearMonthDayHour = (str: string): string => {
  if (!str) return ''
  const date = new Date(str)
  const year = date.getFullYear()
  const month = String(date.getMonth() + 1).padStart(2, '0')
 ...
 * @param {*} amount 计算值
 */
export const amountRule = (amount: string | number, decimal: number) => {
  let defaultAmount = ''
  const setAmount = amount + ''
  if (setAmount !== 'null' && setAmount !== '' && setAmount !== 'undefined' && !isNaN(Number(setAmount))) {
    const num = parseFloat(setAmount)
    if (setAmount.includes('.')) {
      defaultAmount = num.toFixed(decimal)
    } else {
      defaultAmount = Math.floor(num).toString()
    }
    defaultAmount = defaultAmount.replace(/\B(?=(\d{3})+(?!\d))/g, ',')
...
export const flatten = (arr: any[]): any[] => {
  return arr.reduce((conArr: any[], value: any) => {
