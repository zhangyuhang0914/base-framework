<template lang="pug">
.page-view
  Layout(showTabBar showHeaderBar showBack headerBackground="#132B5B" statusBackground="#132B5B" headerColor="#FFFFFF" :headerTitle="query.title")
    template(#main)
      view.layoutMain
...
import { cmsInfo, platformConsultationUrl } from '@/api/index/index'
...
const getDetailInfo = () => {
  ...
  api(query.value.type === 'platformConsultation' ? params : query.value.id)
    .then(value => {
      let data = {}
      if (query.value.type === 'policyGuide' || query.value.type === 'news') {
        data = value.data
